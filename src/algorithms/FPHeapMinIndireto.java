...
public class FPHeapMinIndireto { 
  ...
  public FPHeapMinIndireto (double p[], int v[]) { 
    this.peso = p; this.fp = v; this.n = this.fp.length-1;
  ...
  public void refaz (int esq, int dir) {
    ...
    int x = this.fp[esq];
    while (j <= dir) {
      if ((j < dir) && (this.peso[fp[j]] > this.peso[fp[j + 1]])) j++;
      if (this.peso[x] <= this.peso[fp[j]]) break;
  ...
  public void diminuiChave (int i, double chaveNova) throws Exception {
    ...
    this.peso[x] = chaveNova;
    while ((i > 1) && (this.peso[x] <= this.peso[fp[i / 2]])) {
  ...
  public void imprime () {
    for (int i = 1; i <= this.n; i++)
      System.out.print (this.peso[fp[i]] + " ");
    System.out.println ();
  }
